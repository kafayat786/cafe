...
const Products = () => {
    ...
    const handleDeleteSubmit = async () => {
        try {
            setLoadingCRUD(true);
            axiosWrapper('delete', `${import.meta.env.VITE_API_BASE_URL}/products/${selectedRowId?._id}`);
        } catch (error) {
            return;
        } finally {
            setLoadingCRUD(false);
            setShowDeleteModal(false);
            setProducts(productsData?.filter((val) => val?._id !== selectedRowId?._id));
        }
    };
    ...
    const columns = [
        // { headerName: 'ID', field: 'id', sortable: true, unSortIcon: true, maxWidth: 180 },
        {
            headerName: 'Name',
            field: 'name',
    ...
    return (
        <div className="listing-page">
            <React.Fragment>
                ...
                {productModal.show && (
                    <Dialog header={productModal?.isEditable ? 'Update Products' : 'Add Products'} visible={productModal} style={{ width: '50vw' }} onHide={handleCloseModal}>
                        <ProductsForm fetchData={fetchData} productModal={productModal} resetModal={resetProductModal} />
                    </Dialog>
                )}
                {showDeleteModal && (
                    <ConfirmationBox
                ...
                <Table columns={columns} tableData={productsData} onRowClicked={handleRowClick} createEntry={handleCreateClick} showCreatebtn={true} showExportbtn={false} loading={loading} />
            </React.Fragment>
        </div>
    );
};
